...
export async function POST(req: NextRequest) {
  const db = getDb();
  ensureTable(db);
  const { preferred_letters, preferred_rows, fallback_letters } = await req.json();

  const existing = db.prepare("SELECT id FROM seat_preferences WHERE id = 'default'").get();
  if (existing) {
    db.prepare(
      "UPDATE seat_preferences SET preferred_letters = ?, preferred_rows = ?, fallback_letters = ?, updated_at = datetime('now') WHERE id = 'default'"
    ).run(preferred_letters, preferred_rows, fallback_letters);
  } else {
    db.prepare(
      "INSERT INTO seat_preferences (id, preferred_letters, preferred_rows, fallback_letters) VALUES ('default', ?, ?, ?)"
    ).run(preferred_letters, preferred_rows, fallback_letters);
  }
